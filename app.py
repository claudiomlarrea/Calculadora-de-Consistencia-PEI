    ...
    for _, row in df_raw.iterrows():
        ...
        for i, col_obj, col_act, col_det in blocks:
            ...
            if (pd.notna(obj) and str(obj).strip()) or (pd.notna(act) and str(act).strip()) or (pd.notna(det) and str(det).strip()):
                ...
                tidy_rows.append({
                    "Actividad": actividad_txt if actividad_txt else "",
                    "Obj. actual": str(obj).strip() if pd.notna(obj) else "",
                    "Obj. sugerido": "", **meta
                })
...
def score_and_recommend(df, obj_catalog, col_act, col_obj_actual, col_obj_sug,
    ...
    if SKLEARN_OK:
        perfil_actual = {o: " ".join([normalize_text(t) for t in df.loc[df[col_obj_actual]==o, col_act].astype(str).tolist()]) for o in objetivos} if (col_obj_actual and col_obj_actual in df.columns) else {o:"" for o in objetivos}
        perfil_sugerido = {o: " ".join([normalize_text(t) for t in df.loc[df[col_obj_sug]==o, col_act].astype(str).tolist()]) for o in objetivos} if (col_obj_sug and col_obj_sug in df.columns) else {o:"" for o in objetivos}
...
st.caption("Acepta **Formulario Único crudo** (ancho) o planillas ya estandarizadas. No requiere 'utilis.py'.")
...
file = st.file_uploader("Subí: Formulario Único crudo **o** el Excel con columnas 'Actividad' y 'Obj. actual'.", type=["xlsx","xls"])
